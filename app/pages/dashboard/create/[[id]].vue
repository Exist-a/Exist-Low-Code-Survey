<template>
  <div class="container">
    ...
    <editor class="editor" :isStringOptions :quesSchame="activeQues" :activeNum="activeNum" />
  </div>
</template>
...
const quesComMap: Record<allQuesType, any> = {
  ...
  'mult-select': defineAsyncComponent(() => import('~/components/quesComs/select/multSelect.vue')),
  'text-input': defineAsyncComponent(() => import('~/components/quesComs/textInput/textInput.vue')),
  ...
  scale: defineAsyncComponent(() => import('~/components/quesComs/advancedQuestion/scale.vue')),
  satisfaction: defineAsyncComponent(
    () => import('~/components/quesComs/advancedQuestion/satisfaction.vue')
  ),
  sort: defineAsyncComponent(() => import('~/components/quesComs/advancedQuestion/sort.vue')),
  remark: defineAsyncComponent(() => import('~/components/quesComs/remark/remark.vue')),
  phone: defineAsyncComponent(() => import('~/components/quesComs/contactInfo/phone.vue')),
  ...
  'id-card': defineAsyncComponent(() => import('~/components/quesComs/personalInfo/idCard.vue')),
  gender: defineAsyncComponent(() => import('~/components/quesComs/personalInfo/gender.vue')),
  age: defineAsyncComponent(() => import('~/components/quesComs/personalInfo/age.vue')),
  education: defineAsyncComponent(() => import('~/components/quesComs/personalInfo/education.vue')),
  ...
  industry: defineAsyncComponent(() => import('~/components/quesComs/personalInfo/industry.vue')),
  ...
  wechat: defineAsyncComponent(() => import('~/components/quesComs/contactInfo/wechat.vue'))
}
  ...
  if (survey.value) {
    ...
    if (childrenQues[0] && childrenQues[0].classList.contains('active')) {
      ...
      for (let i = 0; i < childrenQues.length; i++) {
        if (childrenQues[i] && childrenQues[i]?.children[1]?.classList.contains('active')) {
...
const selectQues = (
  ...
  if (isTitle && e.currentTarget && e.currentTarget instanceof HTMLElement) {
    ...
    if (activeQues.value && typeof activeQues.value.state.options.status[0] === 'string') {
      isStringOptions.value = true
    } else if (activeQues.value && typeof activeQues.value.state.options.status[0] !== 'string') {
      isStringOptions.value = false
    } else {
